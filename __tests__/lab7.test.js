describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    const textAfterClick = await page.$eval('product-item', prodItem => {
    ...
    expect(textAfterClick).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountText = await page.evaluate(() => {
      const items = document.getElementsByTagName('product-item');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    const allRemove = await page.$$eval('product-item', prodItem => {
      return prodItem.every(item => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    const allTextReset = await page.$$eval('product-item', prodItems => {
      return prodItems.every(item => {
        return item.shadowRoot.querySelector('button').innerText === "Add to Cart";
      });
    });
    expect(allTextReset).toBe(true);
    const cartCountText = await page.$eval('#cart-count', cartCount => cartCount.innerText);
    expect(cartCountText).toBe('0');
